...
.job{display:flex;align-items:center;gap:8px;font-size:12.5px;color:var(--mu);padding:4px 0}.job i{width:8px;height:8px;border-radius:50%;background:var(--bad2);flex:none}.job.running i{background:var(--warn);animation:blink 1.2s ease-in-out infinite}.job.done i{background:var(--ok)}
...
/* login */
#login{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:radial-gradient(800px 480px at 50% 0%,#2a2119 0%,rgba(42,33,25,0) 70%),var(--bg)}
#login .box{width:100%;max-width:380px;background:var(--sur);border:1px solid var(--bd);border-radius:var(--r);overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,.45)}
#login .in{padding:26px 28px 24px;display:flex;flex-direction:column;gap:10px}#login .mark{width:44px;height:44px;color:var(--ac);margin-bottom:6px}
#login h2{margin:4px 0 2px;font-size:22px;font-weight:700}#login label{margin-top:8px;font-size:12px}
#login input{height:42px;font-size:15px}#login button{height:42px;font-size:15px;margin-top:10px}#lmsg{color:var(--bad2);font-size:12.5px;min-height:18px}
/* phone / narrow */
...
function show(on){document.getElementById('login').style.display=on?'none':'';document.getElementById('shell').style.display=on?'':'none'}
...
function jobRow(label,j,extra){return `<div class="job ${esc(j.state)}"><i></i><span>${label}：${esc(j.msg)}${extra||''}</span></div>`}
async function loadAddons(){try{const d=await api('/api/addons');const el=document.getElementById('addons');
const jobs=Object.entries(d.jobs||{}).map(([id,j])=>jobRow('工坊 '+id,j)).join('')+Object.entries(d.zips||{}).map(([t,j])=>jobRow('打包',j,j.state==='done'?` — <a href="/api/download?token=${t}">下载 ${esc(j.name)}（${j.size_mb} MB）</a>`:'')).join('');
