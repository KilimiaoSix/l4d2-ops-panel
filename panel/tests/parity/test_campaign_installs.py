"""Every install path goes through one gate: only vpks with maps land in addons/. Zip uploads (what gamemaps.com
serves), Workshop search (IPublishedFileService/QueryFiles behind steam_api_key) and the search -> install hand-off."""
import io
import zipfile
...
def test_upload_rejects_vpk_without_maps(api, game_dir):
    r = api.post('/api/upload?name=coach.vpk', content=SKIN_VPK)
    assert r.status_code == 400 and 'maps/*.bsp' in r.json()['error'] and 'materials/' in r.json()['error']
    assert 'coach.vpk' not in game_dir.addon_names() and not list(game_dir.addons.glob('*.uploading'))
...
def test_upload_zip_installs_campaigns_and_reports_the_rest(api, fake_game, game_dir):
    ...
    assert r.status_code == 200, r.text
    j = r.json()
    assert [a['name'] for a in j['installed']] == ['camp.vpk'] and j['installed'][0]['maps'] == ['c1m1_test', 'c1m2_test']
    assert len(j['skipped']) == 1 and j['skipped'][0]['name'] == 'coach.vpk' and 'maps/*.bsp' in j['skipped'][0]['reason']
    assert 'camp.vpk' in game_dir.addon_names() and 'coach.vpk' not in game_dir.addon_names()
...
def test_upload_zip_failures(api, game_dir):
    ...
    assert r.status_code == 400 and 'coach.vpk' in r.json()['error'] and 'maps/*.bsp' in r.json()['error']
    ...
    assert r.status_code == 400 and '.zip' in r.json()['error']
    assert game_dir.addon_names() == before and not list(game_dir.addons.glob('*.uploading'))
...
def test_workshop_search(api, fake_steam):
    ...
    assert q['appid'] == ['550'] and q['requiredtags[0]'] == ['Campaigns'] and q['query_type'] == ['12'] and q['search_text'] == ['helm'] and q['page'] == ['1']
    r = api.get('/api/workshop_search?q=&page=2')
    assert r.status_code == 200 and r.json()['items'] == [] and r.json()['page'] == 2
    q = fake_steam.queries[-1]
    assert q['query_type'] == ['9'] and 'search_text' not in q and q['page'] == ['2']      # empty query = most subscribed campaigns
...
def test_workshop_download_rejects_non_campaign_items(api, panel, fake_steam, game_dir):
    ...
    assert job['state'] == 'error' and 'maps/*.bsp' in job['msg'] and '已保留' not in job['msg']
    assert 'coach.vpk' not in game_dir.addon_names() and not (panel.dir / 'workshop_tmp' / '100200200.part').exists()
