...
<template>
  <section class="view on">
    <div class="card"><h2>在线玩家<span class="sp" /><button class="g sm" @click="session.refreshPlayers()">刷新</button></h2>
      <div class="tw"><table>
        <tr><th>名字</th><th>SteamID</th><th>在线</th><th>延迟</th><th /></tr>
        <tr v-for="p in session.players" :key="p.userid">
          <td><b>{{ p.name }}</b></td><td><code class="mu">{{ p.steamid }}</code></td><td class="mu">{{ p.time }}</td><td class="mu">{{ p.ping }} ms</td>
          <td class="act">
        ...
        <tr v-if="!session.players.length"><td colspan="5" class="mu">当前没有玩家</td></tr>
      </table></div>
    </div>
    <div v-if="session.features?.whitelist" class="card"><h2>白名单<span class="sp" /><span class="mu">{{ wl.length ? wl.length + ' 人' : '空 = 对所有人开放' }}</span></h2>
      ...
      <div class="row"><input v-model="wlId" placeholder="SteamID / 主页链接 / 17位好友码" style="flex:1;min-width:180px" @keydown.enter="add()"><input v-model="wlNote" placeholder="备注" style="width:120px"><button @click="add()">添加</button></div>
      <div>
